...
static void ImDrawList_AddPixelBezier(ImDrawList* drawList, const ImCubicBezierPoints& curve, ImU32 color, float thickness)
{
    lime::buildPixelWire(curve.P0, curve.P1, curve.P2, curve.P3, 1.0f, thickness,
        [drawList, color](ImVec2 lo, ImVec2 hi) { drawList->AddRectFilled(lo, hi, color); });
}
    ...
    bool fill, ImU32 color, float strokeThickness, const ImVec2* startDirHint = nullptr, const ImVec2* endDirHint = nullptr)
{
    ...
    if (fill)
    {
        ImDrawList_AddPixelBezier(drawList, curve, color, thickness);
